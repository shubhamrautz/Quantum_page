...
async function load() {
  const details = await fetchJson("/dashboard/data/details.json");
  

  if (details.notifications)
    notifications_bubble.innerHTML = details.notifications;
  ...
  const coursesinfo = await fetchJson("/dashboard/data/courses-tabs.json");
  ...
  for (let i = 0; i < coursesinfo.coursesdata.length; i++) {
    ...
    if ('favorite' in coursesinfo.coursesdata[i]) {
    ...
    if (
      coursesinfo.coursesdata[i].classes &&
      coursesinfo.coursesdata[i].classes.length > 0
    ) {
    ...
    for (let j = 0; j < 4; j++) fbuttons.childNodes[j].disabled = true;

    if (coursesinfo.coursesdata[i].actions) {
      fbuttons.childNodes[0].disabled =
        !coursesinfo.coursesdata[i].actions.preview;
      fbuttons.childNodes[1].disabled =
        !coursesinfo.coursesdata[i].actions.manage;
      fbuttons.childNodes[2].disabled =
        !coursesinfo.coursesdata[i].actions.grade;
      fbuttons.childNodes[3].disabled =
        !coursesinfo.coursesdata[i].actions.reports;
    }
  }
}
